  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Actualizar máxima y mínima temperatura si es necesario
        if (maxTempEntry && (!targetTempsEntry || maxTempEntry.tempInt1 > targetTempsEntry.maxTempReg)) {
          // Actualizar la máxima temperatura registrada en la base de datos
          await axios.post(import.meta.env.VITE_API_URL + '/api/updateMaxTemp', {
            id: '65b8017e1efb81f1ed066adc',
            maxTempReg: maxTempEntry.tempInt1,
            maxTempDateHour: maxTempEntry.date,
          });
        }

        if (minTempEntry && (!targetTempsEntry || minTempEntry.tempInt1 < targetTempsEntry.minTempReg)) {
          // Actualizar la mínima temperatura registrada en la base de datos
          await axios.post(import.meta.env.VITE_API_URL + '/api/updateMinTemp', {
            id: '65b8017e1efb81f1ed066adc',
            minTempReg: minTempEntry.tempInt1,
            minTempDateHour: minTempEntry.date,
          });
        }
      } catch (error) {
        console.error('Error fetching data:', error);
